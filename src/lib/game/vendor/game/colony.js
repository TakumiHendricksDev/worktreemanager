...
/**
 * The colony: everything that turns WTM's repository/job/session snapshot into a place.
 *
 ...
 *   waiting        → stopped and waiting on you, a bobbing `?` — click it to open the session
 *   long idle      → asleep on the job
 *   anything else  → pottering about its plot
 *
 * Repositories own one persistent hex island each. Every worktree gets a building, agent
 * sessions become astronauts around it, and live shells add a terminal console. Stable WTM ids
 * seed the layout, so refreshing state does not reshuffle the world.
 */

const STALE_MS = 3 * 24 * 60 * 60 * 1000
/** How wide an astronaut is, for the purpose of not fitting through gaps it should not. */
const AGENT_RADIUS = 0.26
/** Progress a live job adds per second, so a working site visibly grows while you watch. */
...
export function statusFor(thread, now = Date.now()) {
  if (thread.gameStatus) return thread.gameStatus
...
export class Colony {
  constructor(scene, settings, camera, renderer) {
    ...
    this.plotOrder = []
    /**
     * Where every zone sits, kept across refreshes and even when a repository has no worktrees.
     * Seeded from WTM's local world layout by `restoreLayout`.
     */
    ...
    // Dismissing the HUD has to survive a refresh: labels are chrome, and state landing while
  ...
  /** What the scatter has to avoid, as one string — cheap to compare every refresh. */
  _plotFootprint() {
  ...
  /**
   * Reshape the colony around a fresh WTM projection. Everything here is keyed by stable
   * repository, worktree and pane ids, so a refresh that changes nothing moves nothing.
   */
  setWorld(snapshot) {
    const repositories = snapshot?.repositories || []
    this.projectNames = new Map(repositories.map((repository) => [repository.id, repository.name]))
    // Every repository gets a persistent island, including an empty or temporarily unavailable one.
    const projects = repositories
      .map((repository) => [repository.id, [...(repository.jobs || [])]])
      .sort((a, b) => {
        if (b[1].length !== a[1].length) return b[1].length - a[1].length
        return (this.projectNames.get(a[0]) || a[0]).localeCompare(this.projectNames.get(b[0]) || b[0])
      })
    ...
    for (const [projectId, list] of projects) {
      ...
      if (!plot) continue
      // Headquarters first, then a stable title order: a new job does not reshuffle the old skyline.
      list.sort((a, b) => {
        if (a.worktree.isMain !== b.worktree.isMain) return a.worktree.isMain ? -1 : 1
        return a.worktree.title.localeCompare(b.worktree.title)
      })

      list.forEach((job, jobIndex) => {
        liveJobs.push(job)
        const building = this._syncBuilding(job, plot, jobIndex)
        seenBuildings.add(job.id)
        this._syncConsole(job, building, plot.accent)
        if (job.shells > 0) seenConsoles.add(job.id)

        job.actors.forEach((actor, actorIndex) => {
          const status = statusFor(actor)
          ...
          const site = this._workSite(plot, building, jobIndex)
          if (job.actors.length > 1) {
            const angle = (actorIndex / job.actors.length) * Math.PI * 2
            site.x += Math.cos(angle) * 0.78
            site.z += Math.sin(angle) * 0.78
          }
          roster.push({
            id: actor.id,
            thread: actor,
    ...
    // A removed worktree retracts its job site; a closed shell takes its console with it.
    for (const [id, entry] of this.buildings) {
      if (!seenBuildings.has(id)) this._removeBuilding(id, entry)
    }
    for (const [id, entry] of this.consoles) {
      if (!seenConsoles.has(id)) this._removeConsole(id, entry)
    }

    this.jobs = new Map(liveJobs.map((job) => [job.id, job]))
    this.threads = new Map(liveActors.map((actor) => [actor.id, actor]))
  ...
  _syncPlots(projects) {
    ...
    // — never because a different repository gained or lost a job. `plotCells` carries it
    // between refreshes, and WTM's stored layout carries it between launches.
    const layout = allocateCells(
      projects.map(([name, list]) => ({ id: name, size: list.length })),
      this.plotCells
    )
    // Remembered, not replaced: a repository that has just lost its last job keeps its
    ...
    projects.forEach(([name], index) => {
      ...
      const label = createLabel(this.projectNames.get(name) || name, accent)
  ...
  _syncBuilding(job, plot, index) {
    let entry = this.buildings.get(job.id)
    // Whole, always. A building that has finished rising is a building you can see all of.
    const target = 1

    if (!entry) {
      const mesh = createBuilding({ seed: hashString(job.id), accent: plot.accent })
      const pos = plot.worldSlot(index)
      mesh.position.copy(pos)
      mesh.rotation.y = ((hashString(job.id) >>> 8) % 360) * (Math.PI / 180)
      ...
      this.buildings.set(job.id, entry)
    } else {
  ...
  /** One compact terminal station per job with at least one live shell. */
  _syncConsole(job, building, accent) {
    if (job.shells <= 0) return
    let entry = this.consoles.get(job.id)
    if (!entry) {
      const group = new THREE.Group()
      group.userData.jobId = job.id
      const body = new THREE.Mesh(
        new THREE.BoxGeometry(0.72, 0.78, 0.52),
        new THREE.MeshStandardMaterial({ color: 0x202936, roughness: 0.62, metalness: 0.28 })
      )
      body.position.y = 0.39
      body.userData.jobId = job.id
      const screen = new THREE.Mesh(
        new THREE.BoxGeometry(0.5, 0.32, 0.025),
        new THREE.MeshBasicMaterial({ color: accent, toneMapped: true })
      )
      screen.position.set(0, 0.48, 0.274)
      screen.userData.jobId = job.id
      group.add(body, screen)
      this.worldGroup.add(group)
      entry = { group, count: 0 }
      this.consoles.set(job.id, entry)
    }
    const anchor = building.mesh.position
    entry.group.position.set(anchor.x + 1.65, anchor.y, anchor.z + 1.2)
    entry.group.rotation.y = Math.atan2(anchor.x - entry.group.position.x, anchor.z - entry.group.position.z)
    entry.count = job.shells
  }

  _removeConsole(id, entry) {
  ...
  _workSite(plot, entry, index) {
    ...
    // are recomputed every refresh, and anything walled in by a neighbour is nudged out to the
  ...
  _growBuildings(dt) {
    for (const [id, entry] of this.buildings) {
      // A newly materialized job site rises instead of appearing in one frame.
  ...
  _isLive(id) {
    ...
    return Boolean(job && job.actors.some((actor) => actor.gameStatus === 'working'))
  }
  ...
  _isActive(id) {
    const job = this.jobs.get(id)
    return Boolean(
      job && job.actors.some((actor) => ['working', 'waiting', 'blocked'].includes(actor.gameStatus))
    )
  }
  ...
  _updateScaffolds() {
    const sites = []
    for (const [id, entry] of this.buildings) {
      // Scaffolding says an agent is actively working here. It used to be
  ...
  /** The worktree building or shell console under the pointer. */
  pickJob(ndcX, ndcY) {
    const ray = this._jobRay || (this._jobRay = new THREE.Raycaster())
    ray.setFromCamera({ x: ndcX, y: ndcY }, this.camera)
    const roots = [
      ...[...this.buildings.values()].map((entry) => entry.mesh),
      ...[...this.consoles.values()].map((entry) => entry.group),
    ]
    const hit = ray.intersectObjects(roots, true)[0]
    let object = hit?.object || null
    while (object && !object.userData?.jobId) object = object.parent
    return object?.userData?.jobId || null
  }
  ...
  dispose() {
    ...
    for (const [id, console] of this.consoles) this._removeConsole(id, console)
