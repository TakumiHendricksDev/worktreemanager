...
# GUI-launched app does not inherit your interactive shell's environment. A .app
# opened from Finder gets PATH=/usr/bin:/bin:/usr/sbin:/sbin, while `just`,
# `acli`, `docker` and `bun` live in /opt/homebrew/bin. Everything works under
# `just dev` and then fails once installed. So doctor checks tools against BOTH
# the current PATH and a login shell's PATH.
set -uo pipefail
...
#
# The fallback must match the macOS arm of `platform::DEFAULT_SHELL` in
# crates/wtm-exec/src/path.rs. If these two disagree, doctor cheerfully reports a
# PATH the app never uses.
login_path="$("${SHELL:-/bin/zsh}" -lc 'printf %s "$PATH"' 2>/dev/null)"

hdr "Platform"

# First thing reported, and a hard failure rather than a warning. wtm is a macOS
# application: the bundle is a .app, `wtm-notify` and `wtm-webview` are Cocoa and
# WebKit FFI, and the release pipeline verifies a Mach-O. Nothing below this line
# is worth reading on another OS, and saying so here is why no recipe in the
# justfile has to.
if [ "$os" = Darwin ]; then
    ok "macOS" "$(sw_vers -productVersion 2>/dev/null || true) $(uname -m)"
else
    bad "platform" "$os — wtm builds and ships on macOS only"
fi
